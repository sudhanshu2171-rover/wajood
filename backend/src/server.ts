...
app.post('/api/wajood/market-bias', (req,res) => {
  const schema=z.object({ phase:z.enum(['PRE_OPEN','OPEN_CONFIRMATION','LIVE']), giftNiftyPct:z.number().nullable(), liveIndexChanges:z.array(z.number()), liveBreadthPct:z.number().nullable().optional() });
